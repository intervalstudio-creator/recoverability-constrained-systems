...
def execute_action_observed(decision: str, context: dict):
    ...
    channels = get_channels_for_decision(decision)

    for channel in channels:
        try:
            if channel == "email" and context.get("email"):
                outputs.append(send_email(context["email"], f"Boundary {decision}", context.get("email_body", f"Decision: {decision}")))
                attempted.append("email")
                mark_success("email")
            elif channel == "sms" and context.get("phone"):
                outputs.append(send_sms(context["phone"], context.get("sms_body", f"Decision: {decision}")))
                attempted.append("sms")
                mark_success("sms")
            elif channel == "incident" and context.get("incident_title"):
                outputs.append(create_incident(context["incident_title"], context.get("incident_description", f"Decision: {decision}"), context.get("incident_severity", "critical")))
                attempted.append("incident")
                mark_success("incident")
            elif channel == "webhook" and context.get("webhook"):
                outputs.append(send_webhook(context["webhook"], {"event": "boundary.execution.decision", "decision": decision, "context": context.get("webhook_context", {})}))
                attempted.append("webhook")
                mark_success("webhook")
            elif channel == "device" and context.get("device_endpoint") and context.get("device_action"):
                outputs.append(trigger_device(context["device_endpoint"], context["device_action"], {"decision": decision}))
