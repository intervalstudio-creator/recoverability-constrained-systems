...
def send_sms(number: str, message: str):
    ...
    if not sid or not token or not sender or not number:
        raise RuntimeError("SMS provider not configured")
    url = f"https://api.twilio.com/2010-04-01/Accounts/{sid}/Messages.json"
    def _send():
        r = requests.post(url, auth=(sid, token), data={"To": number, "From": sender, "Body": message}, timeout=10)
        r.raise_for_status()
        return r.json()
    return {"ok": True, "channel": "sms", "to": number, "provider_response": with_retry(_send, retriable_exceptions=(requests.RequestException,))}
