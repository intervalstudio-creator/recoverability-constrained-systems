...
def create_incident(title: str, description: str, severity: str = "critical"):
    ...
    payload = {"routing_key": routing_key, "event_action": "trigger", "payload": {"summary": title, "severity": severity, "source": "boundary", "custom_details": {"description": description}}}
    def _send():
        r = requests.post("https://events.pagerduty.com/v2/enqueue", json=payload, timeout=10)
        r.raise_for_status()
        return r.json()
    return {"ok": True, "channel": "incident", "provider_response": with_retry(_send, retriable_exceptions=(requests.RequestException,))}
